...
### Product Categories
Now, let's take a look at which product categories tend to have the highest rate of returns. Firstly, we'll use a bar plot to show the proportion of returned goods that came from that product category. Let's sort the data by getting the count of returned goods per product category. In fact, let's just directly convert the data to percentage of returned goods for each product category.

```{r}
bar1 <- returned[, .N/nrow(returned) * 100, by = PRODUCT_CATEGORY_ID]

#Now, let's plot the bar chart using the percentage values.

barplot(bar1[,V1], names = bar1[,PRODUCT_CATEGORY_ID])
```
...
bar2 <- not.returned[, .N/nrow(not.returned) * 100, by = PRODUCT_CATEGORY_ID]

#The same goes for the bar plot.
barplot(bar2[,V1], names = bar2[,PRODUCT_CATEGORY_ID])
```

When trying to compare data using bar plots, it helps if we can do an overlay to see their distributions with respect to each other, especially when the order of the bards are not sorted. Let's try that.

Firstly, you'll realize that product category I has never been returned, and hence does not show up in the original. We'll just fix that by adding in an empty row for I, using the ```rbind``` function.

```{r}
#We'll start by sorting both the datasets based on the product category
bar1 <- rbind(bar1, data.table(PRODUCT_CATEGORY_ID = "I", V1 = 0))
...
```{r, results="hold"}
barplot(bar1[,V1], names = bar1[,PRODUCT_CATEGORY_ID], border = "darkblue", col = "darkblue",  density = 20, ylim = c(0, 30))
par(new=TRUE)
barplot(bar2[,V1], names = bar2[,PRODUCT_CATEGORY_ID], border = "red", col = "red", density = 20, ylim = c(0,30)) 
legend("topright", legend = c("Returned", "Not Returned"), fill = c("darkblue", "red"))
```
