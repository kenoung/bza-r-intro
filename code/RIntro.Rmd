...
However, having a good fot for the regression model is not all that matter. You need to check if your four regression assumptions are met. The four are normality, homoscedasticity, linearity and independence of errors. The independence of errors assumption generally applies to time series data.
...
```{r}
library(car)
durbinWatsonTest(final_fit)
```

The last test is the test for independence of errors. This generally only applies to data collected over a period of time to ensure that each data point is indepedent of each other. For this, we run the Durbin Watson test. As long as the Durbin Watson Statistic is between 1.5 and 2.5, the assumption for independence of errors is valid.
